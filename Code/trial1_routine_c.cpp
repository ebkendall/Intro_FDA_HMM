...
double fn_log_post_continuous(const arma::vec &pars, const arma::field<arma::vec> &prior_par, 
                              ...
                              const arma::field<arma::mat> &K, arma::vec &EIDs) {
    ...
    arma::mat K_1 = K(0);
    arma::mat K_2 = K(1);
    Rcpp::Rcout <<  arma::size(K_1) << std::endl;

    // Rcpp::Rcout << "Evaluate determinant" << std::endl;
    // Rcpp::Rcout << K_1.is_sympd() << std::endl;
    // Rcpp::Rcout << K_2.is_sympd() << std::endl;

    // double log_det_1 = arma::log_det_sympd(K_1);
    // double log_det_2 = arma::log_det_sympd(K_2);

    Rcpp::Rcout << "Calculating inverse1" << std::endl;
    arma::mat precision1 = arma::inv_sympd(K_1);
    Rcpp::Rcout << "Calculating inverse2" << std::endl;
    ...
    // double log_f1_dens = -0.5 * (log_det_1 + interm1);
    // double log_f2_dens = -0.5 * (log_det_2 + interm2);

    // in_value = in_value + log_prior_dens + log_f1_dens + log_f2_dens;

    // Rcpp::Rcout << in_value << ",  " << log_prior_dens << ", " << log_f1_dens << ", " << log_f2_dens << std::endl;
